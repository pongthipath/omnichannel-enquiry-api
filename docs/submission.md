...
- **แอปเดียวรันได้ทั้ง 3 แพลตฟอร์ม** โค้ดชุดเดียว แชร์ service/hook/คอมโพเนนต์กันหมด แต่แยกรูปหน้าจอตามเครื่องที่ใช้ — บนเว็บ/แท็บเล็ตพนักงานได้คอนโซลเมนูซ้าย layout 3 ช่อง บน iOS/Android ทั้งลูกค้าและพนักงานได้แอปมือถือจริง คือแท็บล่างกับหน้าที่ push ซ้อนขึ้นมาแล้วกด back กลับได้
...
`api` กับ `worker` ไม่เก็บ state ในหน่วยความจำเลย เพิ่มจำนวน instance แล้วทำงานได้ทันที Socket.IO คุยข้าม instance ผ่าน Redis adapter ส่วนงานตามเวลาไม่ได้ใช้ lock แยก แต่ให้ฐานข้อมูลเป็นคนตัดสิน — job เช็ค SLA เป็น `UPDATE … WHERE is_sla_breached = false … RETURNING` ก้อนเดียว แถวหนึ่งจึงถูกจับได้โดย worker ตัวเดียวเท่านั้น รันกี่ replica ก็ไม่ส่งแจ้งเตือนซ้ำ ส่วน job มิเรอร์ไฟล์แนบยังเป็น `find()` ธรรมดา ถ้ารันหลายตัวพร้อมกันจะหยิบแถวเดียวกันไปทำซ้ำ (ผลลัพธ์ไม่เพี้ยน เพราะ key ในบักเก็ตผูกกับ id ของไฟล์ แต่เปลืองแรงเปล่า) ตอน deploy จริงควรให้ job นี้จับแถวด้วย `UPDATE … RETURNING` แบบเดียวกัน
...
**บนมือถือเป็นแท็บล่าง 3 อัน** (หน้าแรก / แจ้งเรื่อง / ข้อมูลของฉัน) กดปุ่มลัดแล้ว**ไปหน้าถัดไป**ที่มีปุ่มกลับ ไม่ใช่ modal เพราะฟอร์มบนจอ 390px ที่เด้งเป็นกล่องทับจอเดิมมันอึดอัด แล้วพอคีย์บอร์ดขึ้นมาก็ไม่เหลือที่ให้เห็นว่ากรอกอะไรไปแล้วบ้าง การเปิดเรื่องก็ push เป็นหน้าของตัวเองเหมือนกัน แท็บล่างจะหายไปตอนอยู่ในเรื่อง ให้โฟกัสกับบทสนทนา ส่วนบนเว็บยังเป็น modal เหมือนเดิม — ฟอร์มเดียวกันคนละเปลือก ใช้ `enquiry-draft.tsx` ร่วมกัน จะได้ไม่มีทางที่สองฝั่งกรอกได้ไม่เหมือนกัน
...
**บน iOS/Android เป็นแอปคนละตัวกับหน้าเว็บ** ไม่ใช่เว็บย่อส่วน 3 ช่องเรียงกันบนจอ 390px ไม่มีทางอ่านรู้เรื่อง เลยแยกเป็น:

- **แท็บล่าง 4 อัน** — กล่องงาน (มี badge บอกจำนวนเรื่องที่ยังไม่ปิด) / แดชบอร์ด / ลูกค้า / ฉัน แท็บ "ฉัน" คือที่รวมหน้าตั้งค่าทั้งหมด กับออกจากระบบ ซึ่งบนเว็บอยู่ในเมนูซ้าย
- **ทุกอย่างที่ลึกกว่าแท็บคือหน้า push** — กดเรื่องในกล่องงาน → หน้าบทสนทนา, กดดูข้อมูลลูกค้าจากในเรื่อง → หน้า 360, กดหัวข้อในแท็บฉัน → หน้าตั้งค่านั้น ทุกหน้ามีแถบบนพร้อมปุ่มกลับ และปุ่ม back ของ Android ใช้ได้ตามปกติ แท็บล่างหายไปตอนอยู่ในหน้า push
- **ตารางที่ต้องใช้ความกว้าง** ถูกจัดใหม่ตามความถี่ที่ใช้ — หน้าลูกค้าซึ่งเป็นแท็บหลัก เปลี่ยนจากตาราง 6 คอลัมน์เป็นการ์ดใบละราย (ชื่อร้าน · รหัส · ผู้ติดต่อ · เบอร์, ช่องทางที่ผูกไว้, เซลส์ที่ดูแล, จำนวนเรื่องที่เปิดอยู่, ติดต่อล่าสุด) กดแล้วเข้าหน้า 360 ส่วนหน้า admin ที่นาน ๆ เปิดที (SLA, จัดการสินค้า, แท็ก, พนักงาน) ยังเป็นตารางเดิมแต่เลื่อนแนวนอนได้ คอลัมน์จะได้ไม่ทับกัน

ที่แยกด้วย `Platform.OS` ไม่ใช่ความกว้างหน้าจอ เพราะย่อหน้าต่างเบราว์เซอร์ให้แคบไม่ได้แปลว่ากลายเป็นมือถือ คนทำงานที่ย่อหน้าต่างไว้ครึ่งจอยังอยากได้คอนโซลเดิม ไม่ใช่แท็บล่าง
...
2. **ย้ายงานเบื้องหลังไปที่คิวจริงจัง** ตอนนี้ worker เป็น `setInterval` ที่กันซ้ำด้วยเงื่อนไขใน `UPDATE` เอง ซึ่งพอสำหรับ prototype ของจริงควรเป็น BullMQ หรือ RabbitMQ เต็มรูปแบบ มี dead-letter queue และหน้าจอดูงานที่ค้าง
